...
const rolePermissions=read('supabase/migrations/20260921231500_prestaditos_role_permissions.sql')
...
expect('RLS base inversionistas',base.includes("'inv_investors'")&&base.includes("execute format('alter table public.%I enable row level security',t)"))
expect('RLS inversiones',base.includes("'inv_investments'")&&base.includes("execute format('alter table public.%I enable row level security',t)"))
expect('RLS pagos',base.includes("'inv_payments'")&&base.includes("execute format('alter table public.%I enable row level security',t)"))
...
expect('roles del vertical diferenciados',/OWNER.*ADMIN.*OPERATOR.*READ_ONLY/s.test(rolePermissions))
expect('solo usuarios operativos mutan expedientes',/create policy inv_investor_insert[\s\S]*inv_company_can_operate/i.test(rolePermissions))
expect('solo usuarios operativos mutan storage',/investor_documents_insert[\s\S]*inv_company_can_operate/i.test(rolePermissions))
