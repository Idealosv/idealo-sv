...
const checks=[
 ...
 ['cotizaciones bloquea acciones concurrentes',/\[actionBusy\s*,\s*setActionBusy\]/.test(quotes)&&/const\s+locked\s*=\s*Boolean\(busy\s*\|\|\s*actionBusy\)/.test(quotes)],
 ['cotizaciones convierte OT mediante RPC atómico',/rpc\('convert_quote_to_work_order'/.test(quotes)&&/p_quote_id\s*:\s*form\.id/.test(quotes)],
