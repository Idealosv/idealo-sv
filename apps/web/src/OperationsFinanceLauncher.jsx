...
export default function OperationsFinanceLauncher(){
  ...
  return <>
    ...
    {open&&<div className="erp-modal-backdrop" role="presentation" onMouseDown={()=>setOpen(false)}><section className="erp-modal-panel" role="dialog" aria-modal="true" aria-label="Abastecimiento y finanzas" onMouseDown={e=>e.stopPropagation()}>
      ...
      <div className="erp-modal-body commercial-module">{tab==='Control'&&<ProcurementControlCenter company={company} supabase={supabase} onOpen={selectTab}/>} {tab==='Proveedores'&&<SuppliersModule company={company} supabase={supabase}/>} {tab==='Reposición'&&<ReplenishmentModule company={company} supabase={supabase}/>} {tab==='Recepción'&&<PurchaseReceivingModule company={company} supabase={supabase}/>} {tab==='Compras y gastos'&&<PurchasesExpensesModule company={company} supabase={supabase}/>} {tab==='Cuentas por pagar'&&<SupplierPayablesModule company={company} supabase={supabase}/>} {tab==='Caja'&&<CashModule company={company} supabase={supabase}/>}</div>
    </section></div>}
  </>
}
