...
export default function CashControlCenter({company,supabase,onOpenReports}){
 const [accounts,setAccounts]=useState([]),[moves,setMoves]=useState([]),[recs,setRecs]=useState([]),[clients,setClients]=useState([]),[advances,setAdvances]=useState([]),[workQuotes,setWorkQuotes]=useState([]),[workPayments,setWorkPayments]=useState([]),[collectionRequest,setCollectionRequest]=useState(null),[msg,setMsg]=useState('')
 const load=async()=>{const [a,m,r,c,ad,q,p]=await Promise.all([
  ...
  supabase.from('internal_income_records').select('id,quote_id,work_order_id,amount,received_at').eq('company_id',company.id).order('received_at',{ascending:false}).limit(500)
 ...
 return <section className="cash-control">
  <div className="clients-titlebar cash-simple-title"><div><p className="form-kicker">CAJA</p><h2>Caja y bancos</h2><p>Entradas, salidas y pagos de trabajos.</p></div>{alerts>0&&<button type="button" className="cash-alert-button" onClick={onOpenReports}>{alerts} alerta{alerts===1?'':'s'}</button>}</div>
  ...
  <details className="cash-pending-compact" open>
   <summary><span><strong>Saldos pendientes por cobrar</strong><small>{receivables.length>0?`${receivables.length} trabajo${receivables.length===1?'':'s'} con pago parcial`:'No hay trabajos con saldo pendiente'}</small></span><b>{money(receivablesTotal)}</b></summary>
   {receivables.length>0?<div className="cash-advance-list">{receivables.map(item=><article key={item.id}><div><strong>{item.client_name}</strong><small>{item.prefix||'COT'}-{item.number} · {item.project_name||'Trabajo'} · Pagado {money(item.paid)} de {money(item.total)}</small></div><div><small>Saldo pendiente</small><strong>{money(item.pending)}</strong><button type="button" className="secondary" onClick={()=>collectBalance(item)}>Cobrar saldo</button></div></article>)}</div>:<div className="cash-shift-message">Cuando registres un abono parcial, el saldo restante aparecerá aquí automáticamente.</div>}
  </details>
  {pendingAdvances.length>0&&<details className="cash-pending-compact"><summary><span><strong>Anticipos históricos pendientes</strong><small>{pendingAdvances.length} registro{pendingAdvances.length===1?'':'s'} anterior{pendingAdvances.length===1?'':'es'} por aplicar</small></span><b>{money(k.pending)}</b></summary><div className="cash-advance-list">{pendingAdvances.map(a=>{const pending=Number(a.amount||0)-Number(a.applied_amount||0);return <article key={a.id}><div><strong>{a.clients?.name||'Cliente'}</strong><small>{a.quotes?`${a.quotes.prefix||'COT'}-${a.quotes.number} · ${a.quotes.project_name||'Proyecto'}`:'Sin proyecto'} · {methodLabel(a.payment_method)} · {a.received_at?new Date(a.received_at).toLocaleDateString('es-SV'):'—'}</small></div><div><small>Pendiente</small><strong>{money(pending)}</strong></div></article>})}</div></details>}
