...
export default function CashControlCenter({company,supabase,onOpenReports}){
 const [accounts,setAccounts]=useState([]),[moves,setMoves]=useState([]),[recs,setRecs]=useState([]),[clients,setClients]=useState([]),[advances,setAdvances]=useState([]),[msg,setMsg]=useState('')
 const load=async()=>{const [a,m,r,c,ad]=await Promise.all([
 ...
 ]);const e=a.error||m.error||r.error||c.error||ad.error;if(e)setMsg(e.message);else{setAccounts(a.data||[]);setMoves(m.data||[]);setRecs(r.data||[]);setClients(c.data||[]);setAdvances(ad.data||[])}}
