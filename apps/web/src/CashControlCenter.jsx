...
export default function CashControlCenter({company,supabase,onOpenReports}){
 const [accounts,setAccounts]=useState([]),[moves,setMoves]=useState([]),[recs,setRecs]=useState([]),[clients,setClients]=useState([]),[advances,setAdvances]=useState([]),[workQuotes,setWorkQuotes]=useState([]),[workPayments,setWorkPayments]=useState([]),[msg,setMsg]=useState('')
 ...
 return <section className="cash-control">
  ...
  <details className="cash-pending-compact" open>
   ...
   {receivables.length>0?<div className="cash-advance-list">{receivables.map(item=><article key={item.id}><div><strong>{item.client_name}</strong><small>{item.prefix||'COT'}-{item.number} · {item.project_name||'Trabajo'} · Pagado {money(item.paid)} de {money(item.total)}</small></div><div><small>Saldo pendiente</small><strong>{money(item.pending)}</strong></div></article>)}</div>:<div className="cash-shift-message">Cuando registres un abono parcial, el saldo restante aparecerá aquí automáticamente.</div>}
  </details>
