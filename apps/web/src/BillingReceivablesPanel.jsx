...
const isOpen=(row)=>!['PAID','CANCELLED','VOID'].includes(String(row.status||'').toUpperCase())&&balance(row)>0
const statusLabel=(row)=>{if(!isOpen(row))return 'Pagada';if(row.due_date&&row.due_date<iso())return 'Vencida';if(row.due_date&&row.due_date<=addDays(iso(),7))return 'Por vencer';return 'Pendiente'}
...
export default function BillingReceivablesPanel({company,supabase,onOpenCash}){
  ...
  const load=async()=>{
    ...
    const [ar,pay]=await Promise.all([
      supabase.from('accounts_receivable').select('id,number,source_type,source_id,amount_total,amount_paid,status,due_date,created_at,client_id,clients(name)').eq('company_id',company.id).order('created_at',{ascending:false}),
  ...
  const stats=useMemo(()=>{
    ...
    const overdue=open.filter(row=>row.due_date&&row.due_date<iso())
    const due7=open.filter(row=>row.due_date&&row.due_date>=iso()&&row.due_date<=addDays(iso(),7))
    const due30=open.filter(row=>row.due_date&&row.due_date>=iso()&&row.due_date<=addDays(iso(),30))
    return {
  ...
  const visible=useMemo(()=>{
    ...
    return receivables.filter(row=>{
      ...
      if(filter==='OPEN'&&!isOpen(row))return false
      if(filter==='OVERDUE'&&state!=='Vencida')return false
      if(filter==='DUE7'&&state!=='Por vencer')return false
      if(filter==='PAID'&&isOpen(row))return false
      if(!needle)return true
      return [row.number,row.clients?.name,row.source_type,row.status].some(value=>String(value||'').toLowerCase().includes(needle))
    })
