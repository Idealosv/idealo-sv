   ...
   {tab==='Lotes'&&<>
    <section className="eggs-two-column">
     ...
     <form className="eggs-card eggs-form" onSubmit={classifyBatch}>
      ...
      <Field label="Lote"><select required value={classifyForm.batch_id} onChange={e=>setClassifyForm({...classifyForm,batch_id:e.target.value})}><option value="">Seleccionar</option>{batches.filter(b=>b.status!=='CLOSED').map(b=><option key={b.id} value={b.id}>{b.batch_code} · {number(b.total_eggs)} huevos</option>)}</select></Field>
