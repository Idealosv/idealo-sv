...
export default function Billing360Dashboard({supabase,company,onOpenNewInvoice}){
  ...
  const testGmail=async()=>{if(actionId)return;setActionId('gmail-test');setActionMessage('Probando autenticación y envío real por Gmail…');try{const {data}=await supabase.auth.getSession();const token=data.session?.access_token;if(!token)throw new Error('La sesión venció. Volvé a iniciar sesión.');const response=await fetch(`${apiUrl}/api/dte/gmail-test`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({companyId:company.id})});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.message||`La prueba respondió HTTP ${response.status}.`);setActionMessage(`✓ Gmail conectado. Correo de prueba enviado a ${payload.recipient}. No se generó ni transmitió ningún DTE.`)}catch(cause){setActionMessage(`Prueba de Gmail falló: ${cause.message}`)}finally{setActionId('')}}
