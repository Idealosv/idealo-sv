...
export function WorkOrdersModule({ company, supabase }) {
  ...
  const advance=async(r)=>{const next=nextStatus[r.status];if(!next)return;await supabase.from('work_orders').update({status:next,updated_at:new Date().toISOString()}).eq('id',r.id);await load()}
