...
function openModule(target,tab){window.dispatchEvent(new CustomEvent('idealo-open-module',{detail:{target,tab}}))}
...
export default function ProductionControlCenter({company,supabase}){
  ...
  const load=async(initial=false)=>{
    ...
    try{
      ...
      if(issue)setError('No se pudo actualizar producción. Conservamos la información anterior para que puedas seguir trabajando.')
  ...
  const analysis=useMemo(()=>{
    const now=nowIso(),soon=addHours(72),active=data.orders.filter(openOrder)
    ...
    const enriched=active.map(order=>{const cost=costFor(order.id),profit=Number(order.total||0)-cost,materialRows=data.materials.filter(m=>m.work_order_id===order.id),blockedMaterials=materialRows.filter(m=>Number(m.reserved_qty||0)<Number(m.required_qty||0)&&m.status!=='READY'),openIncidents=data.incidents.filter(i=>i.work_order_id===order.id&&incidentOpen(i)),pendingTasks=data.tasks.filter(t=>t.work_order_id===order.id&&t.status!=='DONE'),delivery=data.deliveries.find(d=>d.work_order_id===order.id&&!['DELIVERED','CANCELLED'].includes(d.status));return {...order,cost,profit,blockedMaterials,openIncidents,pendingTasks,delivery}})
    const late=enriched.filter(o=>o.due_at&&o.due_at<now),dueSoon=enriched.filter(o=>o.due_at&&o.due_at>=now&&o.due_at<=soon),unassigned=enriched.filter(o=>!o.assigned_employee_id&&['DESIGN','APPROVAL','PRODUCTION','REWORK'].includes(o.status)),materialsBlocked=enriched.filter(o=>o.blockedMaterials.length>0&&['PRODUCTION','REWORK','READY'].includes(o.status)),qualityBlocked=enriched.filter(o=>o.openIncidents.length>0||o.quality_status==='REJECTED'),loss=enriched.filter(o=>Number(o.total||0)>0&&o.profit<0).sort((a,b)=>a.profit-b.profit),readyNoDelivery=enriched.filter(o=>o.status==='READY'&&!o.delivery)
    const critical=[...late.map(o=>({type:'late',level:'critical',order:o,title:`OT-${o.number} atrasada`,detail:`${o.title} · ${o.clients?.name||'Cliente'}`})),...loss.map(o=>({type:'loss',level:'critical',order:o,title:`OT-${o.number} con pérdida`,detail:`Resultado ${money(o.profit)} · costo ${money(o.cost)}`})),...materialsBlocked.map(o=>({type:'material',level:'important',order:o,title:`OT-${o.number} bloqueada por materiales`,detail:`${o.blockedMaterials.length} material(es) sin reserva completa`})),...qualityBlocked.map(o=>({type:'quality',level:'important',order:o,title:`OT-${o.number} requiere calidad`,detail:`${o.openIncidents.length} incidencia(s) abierta(s) · ${o.quality_status||'PENDING'}`})),...readyNoDelivery.map(o=>({type:'delivery',level:'important',order:o,title:`OT-${o.number} lista sin entrega programada`,detail:o.clients?.name||o.title}))].slice(0,12)
    const workMinutes=enriched.reduce((sum,o)=>sum+o.pendingTasks.reduce((s,t)=>s+Number(t.estimated_minutes||0),0),0),deadlines=enriched.filter(o=>o.due_at).sort((a,b)=>new Date(a.due_at)-new Date(b.due_at)).slice(0,8)
    return {active,enriched,late,dueSoon,unassigned,materialsBlocked,qualityBlocked,loss,readyNoDelivery,critical,workMinutes,deadlines}
  },[data])

  if(loading)return <section className="production-control panel"><strong>Analizando operación de producción…</strong><small>Estamos organizando órdenes, materiales, calidad y próximas entregas.</small></section>
  return <section className="production-control">
    <div className="production-control-head"><div><p className="form-kicker">CONTROL OPERATIVO</p><h2>Prioridades de producción</h2><p>Qué debe atenderse primero antes de entrar al detalle de cada orden.</p></div><div className="production-control-actions"><button type="button" className="secondary-button" disabled={refreshing} onClick={()=>load(false)}>{refreshing?'Actualizando…':'Actualizar'}</button><button type="button" onClick={()=>openModule('planning')}>Abrir Agenda</button></div></div>
    ...
    <div className="production-control-grid">
      ...
      <section className="panel"><div className="panel-heading"><div><p className="form-kicker">PRÓXIMAS SALIDAS</p><h3>Entregas y fechas límite</h3></div></div>{analysis.deadlines.length?<div className="production-deadline-list">{analysis.deadlines.map(o=><div key={o.id}><time>{new Date(o.due_at).toLocaleString('es-SV',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</time><div><strong>OT-{o.number} · {o.title}</strong><small>{o.clients?.name||'Cliente'} · {o.status} · {o.progress_percent||0}%</small></div><span className={o.due_at<nowIso()?'late':''}>{o.due_at<nowIso()?'ATRASADA':o.priority||'NORMAL'}</span></div>)}</div>:<div className="empty-state"><strong>Sin entregas próximas</strong><small>No hay órdenes activas con fecha límite pendiente. La agenda de producción está despejada.</small></div>}</section>
    </div>
