...
export default function CommercialLauncher() {
 ...
 useEffect(()=>{const fn=e=>{const d=e.detail||{};if(d.target!=='commercial')return;const next=resolveDestination(d.tab||'Productos y trabajos');setContextClient({id:'',name:''});setFlowContext({workOrderId:d.workOrderId||'',quoteId:d.quoteId||''});setTab(next.tab);setOperationView(next.operation);setOpen(true)};window.addEventListener('idealo-open-module',fn);return()=>window.removeEventListener('idealo-open-module',fn)},[])
 useEffect(()=>{const fn=e=>{const detail=e.detail||{};if(!detail.step)return;goJourney(detail.step,detail)};window.addEventListener('idealo-commercial-flow-next',fn);return()=>window.removeEventListener('idealo-commercial-flow-next',fn)},[])
 if(!session||!company)return null
 const openCatalog=()=>{setFlowContext({workOrderId:'',quoteId:''});setTab('Productos y trabajos');window.dispatchEvent(new CustomEvent('idealo-module-change',{detail:'Productos'}))}
