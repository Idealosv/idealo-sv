...
export default function SupplierPayablesModule({company,supabase}){
  const [rows,setRows]=useState([]),[payments,setPayments]=useState([]),[accounts,setAccounts]=useState([]),[message,setMessage]=useState('')
  const [pay,setPay]=useState({payable_id:'',cash_account_id:'',amount:'',payment_method:'CASH',reference:'',notes:''})

  const load=async()=>{
    const [r,p,a]=await Promise.all([
      supabase.from('accounts_payable').select('*,suppliers(name),purchases(number,concept,document_type,document_number,purchase_date)').eq('company_id',company.id).order('created_at',{ascending:false}),
  ...
  const selectPayable=(id)=>{
    ...
    setPay(v=>({...v,payable_id:id,amount:balance?String(balance.toFixed(2)):''}))
  }
  ...
  const registerPayment=async(e)=>{
    ...
    const {error}=await supabase.from('supplier_payments').insert({company_id:company.id,payable_id:r.id,supplier_id:r.supplier_id||null,cash_account_id:pay.cash_account_id,amount,payment_method:pay.payment_method,reference:pay.reference.trim()||null,notes:pay.notes.trim()||null})
    if(error)setMessage(error.message)
    else{setMessage('Pago aplicado al proveedor y salida registrada en caja.');setPay(v=>({...v,payable_id:'',amount:'',reference:'',notes:''}));await load()}
  }

  return <section className="clients-module">
    <div className="clients-titlebar"><div><p className="form-kicker">OBLIGACIONES</p><h2>Cuentas por pagar</h2><p>Controla compras pendientes y paga proveedores desde una caja o cuenta bancaria del ERP.</p></div><span className={overdue>0?'status dte-pending':'status dte-ready'}>{overdue>0?`${money(overdue)} vencido`:'Sin vencidos'}</span></div>
    ...
    {message&&<p className={message.includes('aplicado')?'feedback success':'feedback error'}>{message}</p>}
    <form className="panel client-form-full" onSubmit={registerPayment}>
      ...
      </div><div className="form-actions end"><button disabled={!accounts.length}>Registrar pago y salida de caja</button></div>
