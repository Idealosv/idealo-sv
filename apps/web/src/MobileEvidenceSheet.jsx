...
export default function MobileEvidenceSheet({supabase,company,order,onClose,onSaved,deliveryMode=false,onConfirmDelivery}){
 const [type,setType]=useState(deliveryMode?'DELIVERY':'PRODUCTION'),[files,setFiles]=useState([]),[notes,setNotes]=useState(''),[recipient,setRecipient]=useState(''),[geo,setGeo]=useState(null),[geoError,setGeoError]=useState(''),[busy,setBusy]=useState(false),[msg,setMsg]=useState(''),[gallery,setGallery]=useState(false)
 ...
 const save=async()=>{if(!files.length&&!signed.current){setMsg('Tomá al menos una foto o pedí la firma del cliente.');return}if(deliveryMode&&!recipient.trim()){setMsg('Indicá el nombre de la persona que recibe.');return}setBusy(true);setMsg('');try{let queued=0;for(const f of files){if(await uploadOne(f,type)==='queued')queued++}if(signed.current){const blob=await dataUrlToBlob(canvasRef.current.toDataURL('image/png'));blob.name='firma-cliente.png';if(await uploadOne(blob,'SIGNATURE')==='queued')queued++}
  let deliveryResult=null;if(deliveryMode){if(!onConfirmDelivery)throw new Error('No está disponible la confirmación de entrega.');deliveryResult=await onConfirmDelivery({order,recipient:recipient.trim(),notes:notes||null,geo})}
  if(deliveryMode)setMsg(deliveryResult==='queued'||queued?'Entrega guardada sin conexión. Evidencia y confirmación se sincronizarán automáticamente.':'Entrega confirmada con evidencia correctamente.');else setMsg(queued?'Guardado sin conexión. Se sincronizará automáticamente.':'Evidencia guardada correctamente.')
  if(deliveryResult!=='queued')await onSaved?.();if(!queued&&deliveryResult!=='queued')setGallery(true)
 ...
 return <>{!gallery&&<div className="mobile-sheet-backdrop" onMouseDown={onClose}><section className="mobile-sheet" onMouseDown={e=>e.stopPropagation()}><header><div><small>{deliveryMode?'ENTREGA':'EVIDENCIA'} OT-{order.number}</small><strong>{order.title}</strong></div><button onClick={onClose}>×</button></header><div className="mobile-sheet-body">
  {deliveryMode&&<div className="mobile-evidence-msg">Registrá foto o firma, persona que recibe y GPS antes de cerrar la OT como entregada.</div>}
  <button className="mobile-primary-action" onClick={()=>setGallery(true)}>Ver galería y comprobante</button>
  <label>Tipo de evidencia<select value={type} disabled={deliveryMode} onChange={e=>setType(e.target.value)}><option value="PRODUCTION">Producción</option><option value="INSTALLATION">Instalación</option><option value="DELIVERY">Entrega</option><option value="OTHER">Otra</option></select></label>
  ...
  <label>Persona que recibe{deliveryMode?' *':''}<input value={recipient} onChange={e=>setRecipient(e.target.value)} placeholder="Nombre del cliente o receptor"/></label><label>Notas<textarea rows="3" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Detalle de instalación, entrega o trabajo realizado"/></label>
  ...
  {msg&&<div className="mobile-evidence-msg">{msg}</div>}<button className="mobile-primary-action" disabled={busy} onClick={save}>{busy?'Guardando…':deliveryMode?(navigator.onLine?'Guardar y confirmar entrega':'Guardar entrega sin conexión'):(navigator.onLine?'Guardar evidencia':'Guardar sin conexión')}</button>
 </div></section></div>}{gallery&&<MobileEvidenceGallery supabase={supabase} company={company} order={order} onClose={()=>setGallery(false)}/>}</>
}
