...
export default function MobileEvidenceSheet({supabase,company,order,onClose,onSaved}){
 const [type,setType]=useState('PRODUCTION'),[files,setFiles]=useState([]),[notes,setNotes]=useState(''),[recipient,setRecipient]=useState(''),[geo,setGeo]=useState(null),[geoError,setGeoError]=useState(''),[busy,setBusy]=useState(false),[msg,setMsg]=useState(''),[gallery,setGallery]=useState(false)
 ...
 const save=async()=>{if(!files.length&&!signed.current){setMsg('Tomá al menos una foto o pedí la firma del cliente.');return}setBusy(true);setMsg('');try{let queued=0;for(const f of files){if(await uploadOne(f,type)==='queued')queued++}if(signed.current){const blob=await dataUrlToBlob(canvasRef.current.toDataURL('image/png'));blob.name='firma-cliente.png';if(await uploadOne(blob,'SIGNATURE')==='queued')queued++}setMsg(queued?'Guardado sin conexión. Se sincronizará automáticamente.':'Evidencia guardada correctamente.');await onSaved?.();if(!queued)setGallery(true)}catch(e){if(!navigator.onLine){setMsg('Sin conexión: intentá guardar nuevamente para dejarlo en cola.')}else setMsg(e.message||'No se pudo guardar la evidencia.')}finally{setBusy(false)}}
 return <>{!gallery&&<div className="mobile-sheet-backdrop" onMouseDown={onClose}><section className="mobile-sheet" onMouseDown={e=>e.stopPropagation()}><header><div><small>EVIDENCIA OT-{order.number}</small><strong>{order.title}</strong></div><button onClick={onClose}>×</button></header><div className="mobile-sheet-body">
