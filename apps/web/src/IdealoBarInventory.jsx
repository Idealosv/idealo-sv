...
export default function IdealoBarInventory({company,supabase}){
 ...
 const load=useCallback(async()=>{
  ...
  try{
   const [menuRes,inventoryRes,recipesRes]=await Promise.all([
    supabase.from('bar_menu_items').select('product_id').eq('company_id',companyId).eq('active',true),
    supabase.from('inventory_items').select('id,name,sku,unit,current_stock,average_cost,minimum_stock,active,subcategory,notes').eq('company_id',companyId).eq('active',true).is('deleted_at',null).order('name').limit(1000),
    supabase.from('bar_recipe_components').select('*').eq('company_id',companyId).eq('active',true).order('created_at'),
   ])
   const failure=[menuRes,inventoryRes,recipesRes].find(result=>result.error)?.error
   if(failure)throw failure
   const productIds=[...new Set((menuRes.data||[]).map(row=>row.product_id).filter(Boolean))]
   let nextProducts=[]
   if(productIds.length){
    const productsRes=await supabase.from('finished_products').select('id,name,sku,sale_price,active').in('id',productIds).eq('active',true).order('name')
    if(productsRes.error)throw productsRes.error
    nextProducts=productsRes.data||[]
   }
   const nextRecipes=recipesRes.data||[]
   const recipeInventoryIds=new Set(nextRecipes.map(row=>row.inventory_item_id).filter(Boolean))
   const nextInventory=(inventoryRes.data||[]).filter(row=>recipeInventoryIds.has(row.id)||String(row.subcategory||'').toUpperCase()==='BAR'||String(row.notes||'').toUpperCase().includes('IDEALO BAR'))
   setProducts(nextProducts);setInventory(nextInventory);setRecipes(nextRecipes)
  ...
  }catch(err){setError(String(err?.message||err||'No se pudo cargar inventario y recetas.'))}finally{setLoading(false)}
 },[companyId,supabase])
 ...
 const addRecipe=()=>act(async()=>{
  if(!selectedProductId)throw new Error('Primero agregá un producto en Carta y productos.')
 ...
 const createIngredient=()=>act(async()=>{
  ...
  const {data:item,error:itemError}=await supabase.from('inventory_items').insert({company_id:companyId,name,category:'MATERIAL',subcategory:'BAR',item_type:'MATERIAL',unit,current_stock:0,average_cost:cost,last_cost:cost,standard_cost:cost,active:true,notes:'IDEALO BAR · insumo operativo'}).select('id').single()
  if(itemError)throw itemError
  if(stock>0){
   const {error:movementError}=await supabase.from('inventory_movements').insert({company_id:companyId,inventory_item_id:item.id,movement_type:'INITIAL',quantity:stock,unit_cost:cost,document_type:'BAR_SETUP',document_id:item.id,reference:'IDEALO BAR',notes:'Existencia inicial creada desde Inventario y recetas'})
   if(movementError)throw movementError
  }
  setNewIngredient({name:'',unit:'UNIT',stock:'0',cost:'0'});setMessage('Insumo creado para IDEALO BAR.');await load();setIngredientId(item.id)
 })

 if(loading)return <div className="bar-loading"><span>IDEALO BAR</span><strong>Cargando inventario y recetas…</strong></div>

 return <div className="idealo-bar-shell" style={{paddingTop:8}}>
  <header className="bar-module-head"><div><span>Control de existencias</span><h2>Inventario y recetas</h2><p>Primero registrá los insumos del bar y luego definí cuánto consume cada producto vendido.</p></div><div className="bar-module-stat"><b>{inventory.length}</b><span>insumos del bar</span></div></header>
  ...
  <div className="bar-flow-hint"><div><b>1 · Carta</b><span>Creá lo que vendés.</span></div><div><b>2 · Insumos</b><span>Registrá cerveza e ingredientes.</span></div><div><b>3 · Receta</b><span>Definí el consumo por venta.</span></div><div><b>4 · Venta</b><span>El stock baja automáticamente.</span></div></div>
  <div style={{display:'grid',gridTemplateColumns:'minmax(300px,.85fr) minmax(360px,1.15fr)',gap:14,alignItems:'start'}}>
   <section style={panel}>
    <span className="bar-kicker">1 · Insumos</span><h3 style={{margin:'6px 0 4px',fontSize:22}}>Existencias del bar</h3><p style={{...muted,marginTop:0}}>Aquí solo aparecen insumos creados o usados por IDEALO BAR: cerveza, hielo, pan, carne, queso, papas, aceite, salsas y empaques.</p>
    ...
    <div style={{marginTop:16,maxHeight:300,overflow:'auto'}}>{inventory.length?inventory.map(item=><div key={item.id} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:8,padding:'10px 2px',borderBottom:'1px solid #292f38'}}><div><strong>{item.name}</strong><small style={{display:'block',...muted}}>{item.unit} · costo {money.format(Number(item.average_cost||0))}</small></div><b style={{color:Number(item.current_stock)<=Number(item.minimum_stock)?'#f4b844':'#9ce2bb'}}>{qty(item.current_stock)}</b></div>):<div className="bar-empty-state"><span>＋</span><strong>Sin insumos del bar</strong><p>Creá cerveza, hielo o ingredientes para empezar.</p></div>}</div>
   </section>

   <section style={panel}>
    <span className="bar-kicker">2 · Recetas</span><h3 style={{margin:'6px 0 4px',fontSize:22}}>Consumo por producto</h3><p style={{...muted,marginTop:0}}>Solo aparecen productos de la Carta del bar. Ejemplo: <b style={{color:'#fff'}}>Balde de 6</b> consume 6 cervezas.</p>
    {products.length?<><input style={{...input,marginBottom:8}} placeholder="Buscar producto de la carta…" value={productQuery} onChange={e=>setProductQuery(e.target.value)}/>
