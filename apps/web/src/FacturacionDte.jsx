...
export default function FacturacionDte({session,supabase,company,initialClientId=''}){
  ...
  const setRelatedField=(key,value)=>setRelated(current=>({...current,[key]:value}))
  const setThirdPartyField=(key,value)=>setThirdParty(current=>({...current,[key]:value}))
  const setAppendixField=(key,value)=>setAppendix(current=>({...current,[key]:value}))
      ...
      <div className="billing-classic-columns">
        <div className="billing-classic-main">
          ...
          <details className="billing-technical-details"><summary>Opciones fiscales especiales</summary><div className="billing-fiscal-options">
            ...
            <details><summary>Documento relacionado</summary><div className="form-grid four"><label className="field"><span>Tipo DTE</span><input value={related.tipoDocumento} onChange={e=>setRelatedField('tipoDocumento',e.target.value)}/></label><label className="field"><span>Generación</span><select value={related.tipoGeneracion} onChange={e=>setRelatedField('tipoGeneracion',e.target.value)}><option value="1">Físico</option><option value="2">Electrónico</option></select></label><label className="field"><span>Número / código</span><input value={related.numeroDocumento} onChange={e=>setRelatedField('numeroDocumento',e.target.value)}/></label><label className="field"><span>Fecha</span><input type="date" value={related.fechaEmision} onChange={e=>setRelatedField('fechaEmision',e.target.value)}/></label></div></details>
            <details><summary>Venta a cuenta de tercero</summary><div className="form-grid three"><label className="field"><span>NIT tercero</span><input value={thirdParty.nit} onChange={e=>setThirdPartyField('nit',e.target.value)}/></label><label className="field form-span-2"><span>Nombre tercero</span><input value={thirdParty.nombre} onChange={e=>setThirdPartyField('nombre',e.target.value)}/></label></div></details>
            <details><summary>Apéndice</summary><div className="form-grid three"><label className="field"><span>Campo</span><input value={appendix.campo} onChange={e=>setAppendixField('campo',e.target.value)}/></label><label className="field"><span>Etiqueta</span><input value={appendix.etiqueta} onChange={e=>setAppendixField('etiqueta',e.target.value)}/></label><label className="field"><span>Valor</span><input value={appendix.valor} onChange={e=>setAppendixField('valor',e.target.value)}/></label></div></details>
          </div></details>
        </div>
