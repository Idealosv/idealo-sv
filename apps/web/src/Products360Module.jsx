...
const statuses = [['ACTIVE', 'Activo'], ['INACTIVE', 'Inactivo'], ['TEMPORARY', 'Temporal'], ['DISCONTINUED', 'Descontinuado']]
const sorters = {
...
function totalCost(row) { return number(row.cost_estimate) + number(row.labor_cost) + number(row.installation_cost) }
...
export default function Products360Module({ company, supabase }) {
  ...
  const stats = useMemo(() => {
    ...
    return { total: rows.length, active: active.length, avgMargin, noCost }
  }, [rows])
  ...
  const reset = () => {
    setForm(emptyForm()); setVariants([]); setTiers([]); setAdvancedOpen(false)
    setVariantForm({ name: '', sku: '', attributes_text: '', sale_price: '', cost_estimate: '' })
    setTierForm({ min_quantity: '', max_quantity: '', unit_price: '', notes: '' })
    setMessage({ type: '', text: '' })
  }
  ...
  const save = async event => {
    ...
    const request = form.id
      ? supabase.from('finished_products').update(payloadFromForm()).eq('id', form.id).eq('company_id', company.id).select().single()
      : supabase.from('finished_products').insert(payloadFromForm()).select().single()
    const { data, error } = await request
    if (error) setMessage({ type: 'error', text: error.message.includes('duplicate') ? 'Ese SKU ya existe en la empresa.' : error.message })
    else {
      await load(); edit(data)
  ...
  const duplicate = async row => {
    ...
    copy.name = `${row.name} (copia)`; copy.sku = skuSeed(row.name)
  ...
  const toggleActive = async row => {
    ...
    if (error) setMessage({ type: 'error', text: error.message })
    else { await load(); if (form.id === row.id) edit({ ...row, active: !activeNow, status: activeNow ? 'INACTIVE' : 'ACTIVE' }) }
  }
  ...
  const remove = async row => {
    ...
    if (number(quoteCount) + number(workCount) > 0) {
      ...
      setMessage(error ? { type: 'error', text: error.message } : { type: 'success', text: 'El producto tiene historial comercial; se marcó como descontinuado.' })
    } else {
  ...
  const addVariant = async () => {
    ...
    if (error) setMessage({ type: 'error', text: error.message })
    else { setVariantForm({ name: '', sku: '', attributes_text: '', sale_price: '', cost_estimate: '' }); await loadChildren(form.id) }
  }
  ...
  const addTier = async () => {
    ...
    if (error) setMessage({ type: 'error', text: error.message })
    else { setTierForm({ min_quantity: '', max_quantity: '', unit_price: '', notes: '' }); await loadChildren(form.id) }
  }
  ...
  return <section className="products360 products360-simple">
    <div className="products360-hero">
      <div><p className="form-kicker">PRODUCTOS</p><h2>Catálogo</h2></div>
      <div className="products360-hero-actions"><button type="button" onClick={reset}>+ Nuevo producto</button></div>
    </div>

    {message.text && <p className={`feedback ${message.type === 'error' ? 'error' : 'success'}`}>{message.text}</p>}

    <div className="products360-summary">
      <span><strong>{stats.active}</strong> activos</span>
      <span><strong>{stats.total}</strong> total</span>
      {stats.noCost > 0 && <span className="warning"><strong>{stats.noCost}</strong> sin costo</span>}
      {stats.active > 0 && <span><strong>{stats.avgMargin.toFixed(0)}%</strong> margen medio</span>}
    </div>
    ...
    <section className="panel products360-toolbar">
      ...
      <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}><option value="ALL">Categorías</option>{categories.map(c => <option key={c}>{c}</option>)}</select>
      <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="ALL">Estados</option>{statuses.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
      <select value={sortBy} onChange={e => setSortBy(e.target.value)}><option value="name">Nombre</option><option value="recent">Recientes</option><option value="price_desc">Precio ↓</option><option value="price_asc">Precio ↑</option><option value="margin_desc">Margen</option></select>
      <span className="products360-result-count">{filtered.length}</span>
    </section>

    <section className="products360-layout">
      <section className="panel products360-list-panel">
        <div className="products360-list-title"><strong>Productos</strong><span>{filtered.length}</span></div>
        {filtered.length ? <div className="products360-list">{filtered.map(row => <article key={row.id} className={`product360-row ${form.id === row.id ? 'selected' : ''}`} onClick={() => edit(row)}>
          ...
          <div className="product360-main"><div className="product360-titleline"><strong>{row.name}</strong></div><p>{row.category || 'Sin categoría'} · {row.unit || 'unidad'}</p></div>
          <div className="product360-finance"><strong>{money(row.sale_price)}</strong><small>{marginOf(row).toFixed(0)}% margen</small></div>
        </article>)}</div> : <div className="empty-state"><strong>Sin productos</strong><p>Creá el primero con el formulario.</p></div>}
      </section>

      <form className="panel products360-editor" onSubmit={save}>
        <div className="products360-editor-head-simple">
          <div><small>{form.id ? 'EDITANDO' : 'NUEVO PRODUCTO'}</small><h3>{form.id ? form.name : 'Datos principales'}</h3></div>
          <button type="button" className="text-action" onClick={() => setAdvancedOpen(v => !v)}>{advancedOpen ? 'Ocultar opciones' : 'Más opciones'}</button>
        </div>

        <div className="products360-basic-grid">
          <label className="field product-name"><span>Nombre *</span><input required value={form.name} onChange={e => update('name', e.target.value)} placeholder="Ej. Camisa personalizada" /></label>
          ...
          <label className="field"><span>Unidad</span><select value={form.unit} onChange={e => update('unit', e.target.value)}>{units.map(u => <option key={u}>{u}</option>)}</select></label>
          ...
          <label className="field"><span>Costo estimado</span><input type="number" min="0" step="0.01" value={form.cost_estimate} onChange={e => update('cost_estimate', e.target.value)} /></label>
          <label className="field product-description"><span>Descripción</span><input value={form.short_description} onChange={e => update('short_description', e.target.value)} placeholder="Descripción breve para cotizaciones" /></label>
        </div>

        <div className="product360-quick-profit">
          <span>Costo <strong>{money(formCost)}</strong></span>
          <span>Utilidad <strong>{money(formProfit)}</strong></span>
          <span>Margen <strong className={formMargin < 25 ? 'danger-text' : ''}>{formMargin.toFixed(1)}%</strong></span>
        </div>

        {advancedOpen && <div className="products360-advanced">
          <div className="form-section-title">Identificación adicional</div>
          <div className="form-grid three">
            <label className="field"><span>SKU / código</span><input value={form.sku} onChange={e => update('sku', e.target.value.toUpperCase())} placeholder="Automático" /></label>
            ...
            <label className="field form-span-2"><span>Etiquetas</span><input value={form.tags_text} onChange={e => update('tags_text', e.target.value)} placeholder="premium, urgente, exterior" /></label>
            ...
            <label className="field form-span-3"><span>Especificación técnica</span><textarea rows="2" value={form.technical_description} onChange={e => update('technical_description', e.target.value)} /></label>
          </div>

          <div className="form-section-title">Costos e impuestos</div>
          <div className="form-grid four">
            ...
            <label className="field"><span>Instalación</span><input type="number" min="0" step="0.01" value={form.installation_cost} onChange={e => update('installation_cost', e.target.value)} /></label>
          ...
          <div className="form-section-title">Medidas y producción</div>
          <div className="form-grid four">
            ...
            <label className="field"><span>Unidad medida</span><select value={form.dimension_unit} onChange={e => update('dimension_unit', e.target.value)}><option value="m">Metros</option><option value="cm">Centímetros</option><option value="mm">Milímetros</option></select></label>
            <label className="field"><span>Área</span><input readOnly value={`${calculatedArea.toFixed(4)} m²`} /></label>
            <label className="field"><span>Tiempo (min)</span><input type="number" min="0" value={form.estimated_minutes} onChange={e => update('estimated_minutes', e.target.value)} /></label>
            <label className="field"><span>Plazo (días)</span><input type="number" min="0" value={form.lead_time_days} onChange={e => update('lead_time_days', e.target.value)} /></label>
            <label className="field form-span-2"><span>Imagen URL</span><input type="url" value={form.image_url} onChange={e => update('image_url', e.target.value)} placeholder="https://..." /></label>
            <label className="field form-span-4"><span>Notas internas</span><textarea rows="2" value={form.internal_notes} onChange={e => update('internal_notes', e.target.value)} /></label>
          </div>
          {suggestedM2Price > 0 && <div className="product360-calculator"><span>Precio por superficie: <strong>{money(suggestedM2Price)}</strong></span><button type="button" className="secondary-action" onClick={applyM2}>Usar precio</button></div>}
          ...
          {form.image_url && <div className="product360-image-preview"><img src={form.image_url} alt="Referencia" onError={e => { e.currentTarget.style.display = 'none' }} /></div>}

          {form.id && <div className="products360-subgrid products360-subgrid-inline">
            <section>
              <div className="products360-list-title"><strong>Variantes</strong><span>{variants.length}</span></div>
              <div className="form-grid two compact-grid"><label className="field"><span>Nombre</span><input value={variantForm.name} onChange={e => setVariantForm({ ...variantForm, name: e.target.value })} /></label><label className="field"><span>Precio</span><input type="number" step="0.01" value={variantForm.sale_price} onChange={e => setVariantForm({ ...variantForm, sale_price: e.target.value })} /></label></div>
              <div className="form-actions end"><button type="button" onClick={addVariant}>Agregar variante</button></div>
              {variants.map(v => <div className="mini-table-row" key={v.id}><div><strong>{v.name}</strong><small>{v.sale_price == null ? 'Precio base' : money(v.sale_price)}</small></div><button type="button" className="text-danger-action" onClick={() => deleteVariant(v.id)}>Quitar</button></div>)}
            </section>
            <section>
              <div className="products360-list-title"><strong>Precios por cantidad</strong><span>{tiers.length}</span></div>
              <div className="form-grid two compact-grid"><label className="field"><span>Desde</span><input type="number" min="0.01" step="0.01" value={tierForm.min_quantity} onChange={e => setTierForm({ ...tierForm, min_quantity: e.target.value })} /></label><label className="field"><span>Precio</span><input type="number" min="0" step="0.01" value={tierForm.unit_price} onChange={e => setTierForm({ ...tierForm, unit_price: e.target.value })} /></label></div>
              <div className="form-actions end"><button type="button" onClick={addTier}>Agregar escala</button></div>
              {tiers.map(t => <div className="mini-table-row" key={t.id}><div><strong>Desde {t.min_quantity}</strong><small>{money(t.unit_price)} / {form.unit}</small></div><button type="button" className="text-danger-action" onClick={() => deleteTier(t.id)}>Quitar</button></div>)}
            </section>
          </div>}
        </div>}

        <div className="form-actions products360-savebar">
          {form.id && <button type="button" className="secondary-action" onClick={() => duplicate(form)} disabled={busy}>Duplicar</button>}
          {form.id && <button type="button" className="secondary-action" onClick={() => toggleActive(form)} disabled={busy}>{form.status === 'ACTIVE' ? 'Desactivar' : 'Activar'}</button>}
          {form.id && <button type="button" className="danger-action" onClick={() => remove(form)} disabled={busy}>Eliminar</button>}
          <button disabled={busy}>{busy ? 'Guardando…' : form.id ? 'Guardar cambios' : 'Crear producto'}</button>
        </div>
      </form>
    </section>
  </section>
