...
export default function SaasMasterPanelHost() {
  ...
  const createCompany = async event => {
    ...
    try {
      ...
      setNotice('Empresa y membresía creadas correctamente.')
      await reload()
    } catch (err) {
      setError(err.message)
    } finally {
  ...
  const openPayment = row => {
    const suggested = Number(row.subscription?.plan?.monthly_price || 0)
    setPaymentFor(row)
    setPayment({ amount: suggested ? String(suggested) : '', reference: '' })
    setError('')
  }
  ...
  const recordPayment = async event => {
    ...
    try {
      await request(`/api/admin/saas/companies/${paymentFor.id}/payments`, {
        method: 'POST',
        body: JSON.stringify({ amount, reference: payment.reference.trim() }),
      })
      ...
      setPaymentFor(null)
      setNotice(`Pago registrado y membresía de ${paymentFor.name} renovada por 30 días.`)
      await reload()
    } catch (err) {
      setError(err.message)
    } finally {
    ...
    <div className="saas-master-root">
      <header className="saas-master-header">
        <div>
          ...
          <p>Control de empresas, planes, cobros, vencimientos y acceso al sistema.</p>
        </div>
      ...
      {loading ? <section className="saas-master-message">Cargando membresías…</section> : data && <>
        <section className="saas-master-metrics">
          ...
          <Metric label="MRR estimado" value={`$${Number(m.mrr || 0).toFixed(2)}`} hint="mensual" tone="money" />
        </section>
        ...
        <section className="saas-master-grid">
          <form className="saas-master-card saas-master-create" onSubmit={createCompany}>
            ...
            <label>Plan<select value={form.plan_id} onChange={e => setForm({ ...form, plan_id: e.target.value })}>{data.plans.map(x => <option key={x.id} value={x.id}>{x.name} · ${Number(x.monthly_price).toFixed(2)}/mes</option>)}</select></label>
          ...
          <section className="saas-master-card saas-master-list">
            ...
            <div className="saas-master-table-wrap">
              <table>
                <thead><tr><th>Empresa</th><th>Plan</th><th>Estado</th><th>Vencimiento</th><th>Usuarios</th><th>Acciones</th></tr></thead>
      ...
      {paymentFor && <div className="saas-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && !saving && setPaymentFor(null)}>
        <form className="saas-payment-modal" onSubmit={recordPayment}>
          ...
          <div className="saas-payment-plan"><span>Plan actual</span><strong>{paymentFor.subscription?.plan?.name || 'Sin plan'}</strong><span>Mensualidad</span><strong>${Number(paymentFor.subscription?.plan?.monthly_price || 0).toFixed(2)}</strong></div>
          ...
          <p>Al guardar el pago, la membresía quedará <strong>activa</strong> y se renovará por <strong>30 días</strong>.</p>
          <div className="saas-payment-actions"><button type="button" onClick={() => setPaymentFor(null)} disabled={saving}>Cancelar</button><button type="submit" disabled={saving}>{saving ? 'Registrando…' : 'Registrar pago y renovar'}</button></div>
        </form>
      </div>}
    </div>
  )
...
function CompanyRow({ row, plans, saving, onUpdate, onPayment }) {
  ...
  return <tr>
    ...
    <td><select value={sub?.plan_id || ''} disabled={!sub || saving} onChange={e => onUpdate(row.id, { plan_id: e.target.value }, 'Plan actualizado.')}>{!sub && <option value="">Sin plan</option>}{plans.map(x => <option key={x.id} value={x.id}>{x.name} · ${Number(x.monthly_price).toFixed(2)}</option>)}</select></td>
    ...
    <td>{row.users}</td>
    <td><div className="saas-master-row-actions"><button type="button" disabled={!sub || saving} onClick={() => onPayment(row)}>Registrar pago</button><button type="button" disabled={!sub || saving} onClick={() => onUpdate(row.id, { status: 'active', renew: true }, 'Membresía renovada por 30 días.')}>Renovar 30 días</button>{sub?.status !== 'suspended' ? <button type="button" className="danger" disabled={!sub || saving} onClick={() => onUpdate(row.id, { status: 'suspended' }, 'Membresía suspendida.')}>Suspender</button> : <button type="button" disabled={!sub || saving} onClick={() => onUpdate(row.id, { status: 'active' }, 'Membresía reactivada.')}>Reactivar</button>}</div></td>
  </tr>
}
