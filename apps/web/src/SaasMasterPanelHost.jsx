...
export default function SaasMasterPanelHost(){
 ...
 const [form,setForm]=useState({name:'',owner_email:'',plan_id:'',vertical_id:'',trial_days:14})
 ...
 const rows=useMemo(()=>{const term=search.trim().toLowerCase();if(!term)return data?.companies||[];return(data?.companies||[]).filter(row=>`${row.name} ${row.slug} ${row.subscription?.plan?.name||''} ${row.subscription?.vertical?.name||''}`.toLowerCase().includes(term))},[data,search])
 ...
 return <div className="saas-master-root">
  <header className="saas-master-header"><div><span className="saas-master-brand">IDEALO SV</span><h1>Panel Maestro</h1><p>Empresas, rubros, planes, vencimientos y control comercial.</p></div><div className="saas-master-actions"><a href="/">Volver al ERP</a><button onClick={reload} disabled={loading}>Actualizar</button></div></header>
