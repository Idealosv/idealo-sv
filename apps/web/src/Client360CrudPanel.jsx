...
export default function Client360CrudPanel({ supabase, companyId, client, data, onChanged }) {
  ...
  const saveContact = async (event) => {
    ...
    setContact(emptyContact); await finish(editing ? 'Contacto actualizado.' : 'Contacto agregado.')
  }
  ...
  const saveAddress = async (event) => {
    ...
    setAddress(emptyAddress); await finish(editing ? 'Dirección actualizada.' : 'Dirección agregada.')
  }
  ...
  const saveInteraction = async (event) => {
    ...
    setInteraction(emptyInteraction); await finish(editing ? 'Seguimiento actualizado.' : 'Seguimiento registrado.')
  }
  ...
  const saveCredit = async (event) => {
    ...
    if (error) return fail(error)
    const { error: clientError } = await supabase.from('clients').update({ credit_limit: payload.credit_limit, credit_days: payload.credit_days, blocked_for_debt: payload.blocked, updated_at: new Date().toISOString() }).eq('id', client.id).eq('company_id', companyId)
