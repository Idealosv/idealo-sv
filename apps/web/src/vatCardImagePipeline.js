...
export function preprocessVatField(source, mode = 'text') {
  ...
  for (let i = 0; i < image.data.length; i += 4) {
    ...
    const stretched = clamp(Math.round((gray - low) * 255 / span), 0, 255)
    const value = mode === 'digits' ? clamp(Math.round((stretched - 128) * 1.08 + 128), 0, 255) : stretched
  ...
  try {
    ...
    for (let p = Math.round(length * .30); p <= Math.round(length * .70); p += 1) {
      ...
      for (let c = 0; c < cross; c += 4) {
        const x = horizontal ? c : p; const y = horizontal ? p : c; const i = (y * sample.width + x) * 4
...
// El contorno se localiza por densidad de bordes, no por color del fondo.
// Esto tolera fondo blanco/gris/crema, cambios de contraste y márgenes externos.
function detectCardRectByEdges(bitmap, rect) {
  ...
  try {
    ...
    for (let y = 0; y < sample.height; y += 1) for (let x = 0; x < sample.width; x += 1) {
      ...
      gray[y * sample.width + x] = Math.round(image.data[i] * .299 + image.data[i + 1] * .587 + image.data[i + 2] * .114)
    }
    ...
    for (let y = 2; y < sample.height - 2; y += 2) {
      for (let x = 2; x < sample.width - 2; x += 2) {
        ...
        if (gx + gy > 35) { row[y] += 1; col[x] += 1 }
      }
    }
    for (let y = 0; y < sample.height; y += 2) {
      row[y] /= Math.max(1, Math.ceil(sample.width / 2)); if (y + 1 < row.length) row[y + 1] = row[y]
    }
    for (let x = 0; x < sample.width; x += 2) {
      col[x] /= Math.max(1, Math.ceil(sample.height / 2)); if (x + 1 < col.length) col[x + 1] = col[x]
    }
    const yBand = activeBand(smooth(row, 15), .015)
    const xBand = activeBand(smooth(col, 15), .015)
    if (!xBand || !yBand) return rect
    let [left, right] = xBand; let [top, bottom] = yBand
    const dw = right - left; const dh = bottom - top; const ratio = dw / Math.max(1, dh)
    if (dw < sample.width * .35 || dh < sample.height * .30 || ratio < 1.15 || ratio > 1.95) return rect
    const mx = Math.round(dw * .015); const my = Math.round(dh * .02)
    left = clamp(left - mx, 0, sample.width - 2); right = clamp(right + mx, left + 2, sample.width)
    top = clamp(top - my, 0, sample.height - 2); bottom = clamp(bottom + my, top + 2, sample.height)
...
function estimateBackground(image) {
  const { width, height, data } = image; const pw = Math.max(8, Math.round(width * .07)); const ph = Math.max(8, Math.round(height * .07))
  const starts = [[0, 0], [width - pw, 0], [0, height - ph], [width - pw, height - ph]]
  let r = 0; let g = 0; let b = 0; let n = 0
  starts.forEach(([sx, sy]) => { for (let y = sy; y < sy + ph; y += 3) for (let x = sx; x < sx + pw; x += 3) { const i = (y * width + x) * 4; r += data[i]; g += data[i + 1]; b += data[i + 2]; n += 1 } })
  return [r / n, g / n, b / n]
}
