...
export default function ProductionCalendarLauncher(){
 ...
 return <><button type="button" onClick={()=>setOpen(true)} className="sidebar-module-access planning"><span className="module-glyph">▦</span><span className="module-copy"><span>Planificación</span><small>Calendario · Personal · Entregas</small></span></button>{open&&<div className="erp-modal-backdrop" role="presentation" onMouseDown={()=>setOpen(false)}><section className="erp-modal-panel" role="dialog" aria-modal="true" aria-label="Planificación de producción" onMouseDown={e=>e.stopPropagation()}><header className="erp-modal-head"><div><strong>Planificación de producción</strong><small>Órdenes → agenda → equipo → entregas e instalaciones</small></div><button type="button" className="erp-modal-close" onClick={()=>setOpen(false)}>×</button></header><div className="erp-modal-body commercial-module"><ProductionCalendar company={company} supabase={supabase}/></div></section></div>}</>
}
