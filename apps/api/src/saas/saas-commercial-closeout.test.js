...
test('compatibilidad del panel no duplica renovación ya aplicada por el cobro atómico',()=>{assert.match(safeSubscription,/wasJustRenewedByMonthlyPayment/);assert.match(safeSubscription,/paymentAge>30000/);assert.match(safeSubscription,/charge_type\|\|'\)\s*!==\s*'monthly'/);assert.match(safeSubscription,/renewal_already_applied:true/)})
