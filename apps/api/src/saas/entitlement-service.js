...
export async function getCompanyEntitlements({request,supabase}){
 ...
 if(!subscription)return{company_id:companyId,role:membership.role,access:false,status:'unconfigured',reason:'La empresa no tiene una membresía configurada.',modules:[],plan:null,usage:{users:1,max_users:1}}
 ...
 const [{data:planModules,error:pmError},{data:overrides,error:ovError},{count:users,error:userCountError}]=await Promise.all([
 ...
 ]);if(pmError)throw pmError;if(ovError)throw ovError;if(userCountError)throw userCountError
 ...
 const status=String(subscription.status||'');const now=Date.now();const graceValid=status==='past_due'&&subscription.grace_ends_at&&new Date(subscription.grace_ends_at).getTime()>now;const access=['trial','active'].includes(status)||graceValid
 const modules=access?[...moduleMap.entries()].filter(([,enabled])=>enabled).map(([code])=>code):[]
 return{company_id:companyId,role:String(membership.role||''),access,status,reason:access?null:(status==='past_due'?'El período de gracia de la membresía finalizó.':'La membresía está suspendida o cancelada.'),plan:plan?{id:plan.id,code:plan.code,name:plan.name,max_users:plan.max_users,max_branches:plan.max_branches,dte_enabled:plan.dte_enabled,ai_enabled:plan.ai_enabled,mobile_apps_enabled:plan.mobile_apps_enabled,monthly_price:plan.monthly_price,activation_fee:plan.activation_fee}:null,modules,usage:{users:Number(users||0),max_users:Number(plan?.max_users||0)},subscription:{status:subscription.status,trial_ends_at:subscription.trial_ends_at,current_period_end:subscription.current_period_end,grace_ends_at:subscription.grace_ends_at,activation_paid_at:subscription.activation_paid_at}}
}
