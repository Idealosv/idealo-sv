...
function required(value, name) {
  if (value === undefined || value === null || String(value).trim() === '') {
    throw new Error(`${name} es obligatorio para el DTE-01.`)
  }
  return value
}
...
function dateParts(date) {
  ...
  return {
    ...
    time: `${pad(local.getHours())}:${pad(local.getMinutes())}:${pad(local.getSeconds())}`,
  }
}
...
function buildItem(item, index) {
  ...
  if (!(quantity > 0) || unitPrice < 0 || discount < 0) {
    throw new Error(`Los importes del ítem ${index + 1} no son válidos.`)
  }
  const taxable = money(quantity * unitPrice - discount)
  if (taxable < 0) throw new Error(`El descuento del ítem ${index + 1} excede su valor.`)
  ...
  return {
    ...
    psv: 0,
    noGravado: 0,
    ivaItem: money(taxable - taxable / 1.13),
  }
}
...
export function buildFacturaElectronica({
  ...
  if (!Array.isArray(items) || items.length === 0 || items.length > 2000) {
    throw new Error('El DTE-01 debe incluir entre 1 y 2000 partidas.')
  }
  ...
  const dte = {
    identificacion: {
      ...
      ambiente: '00',
      tipoDte: '01',
      numeroControl: String(required(numeroControl, 'numeroControl')),
    ...
    emisor: {
      nit: String(required(emisor?.nit, 'NIT del emisor')),
      nrc: String(required(emisor?.nrc, 'NRC del emisor')),
      nombre: String(required(emisor?.nombre, 'nombre del emisor')),
      codActividad: String(required(emisor?.codActividad, 'actividad del emisor')),
      descActividad: String(required(emisor?.descActividad, 'descripción de actividad del emisor')),
      nombreComercial: emisor.nombreComercial || null,
      direccion: address(emisor.direccion, 'emisor'),
      telefono: String(required(emisor?.telefono, 'teléfono del emisor')),
      correo: String(required(emisor?.correo, 'correo del emisor')),
    ...
    receptor: receptor ? {
      ...
      direccion: receptor.direccion ? address(receptor.direccion, 'receptor') : null,
    ...
    resumen: {
      ...
      totalLetras: String(required(totalLetras, 'total en letras')),
      totalIva,
      saldoFavor: 0,
