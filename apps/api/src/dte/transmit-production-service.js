...
function buildPublicMhError(error, phase = 'recepcion') {
  ...
  const detail = [value.mensaje, value.message, value.descripcionMsg, value.descripcion, value.detalle, value.error]
    .find((item) => typeof item === 'string' && item.trim())?.trim().slice(0, 500)
  const publicError = new Error(detail
    ? `Hacienda PRODUCCIÓN rechazó la ${phase === 'autenticacion' ? 'autenticación' : 'recepción del DTE'}: ${detail}`
    : `Hacienda PRODUCCIÓN rechazó la ${phase === 'autenticacion' ? 'autenticación' : 'recepción del DTE'}${error?.status ? ` (HTTP ${error.status})` : ''}.`)
...
export function productionConfirmation(controlNumber) {
  return `TRANSMITIR PRODUCCION ${String(controlNumber || '').trim()}`
}

export function buildProductionReceptionPayload(document, attemptNumber = 1) {
  return {
    ambiente: '01',
...
export async function transmitSignedProductionDte({ request, supabase, env = process.env, fetchImpl = fetch }) {
  ...
  const preflight = getDteProductionPreflightStatus(env)
  ...
  if (config.environment !== 'production' || !config.mhBaseUrl.includes('api.dtes.mh.gob.sv')) {
    const error = new Error('La API no está configurada para el endpoint oficial de PRODUCCIÓN de Hacienda.')
    error.statusCode = 409
    throw error
  }
  ...
  const { data: membership, error: membershipError } = await supabase
    .from('company_members').select('role').eq('company_id', document.company_id).eq('user_id', userData.user.id).maybeSingle()
  ...
  if (document.environment !== 'production' || document.dte_payload?.identificacion?.ambiente !== '01') {
    const error = new Error('Este endpoint acepta exclusivamente DTE preparados para ambiente PRODUCCIÓN 01.')
    error.statusCode = 409
    throw error
  }
  ...
  // Reclamo atómico del documento: solo una solicitud puede cambiar SIGNED -> TRANSMITTING.
  ...
  const { data: existingAttempts, error: attemptsError } = await supabase
    ...
    .select('id, attempt_number, finished_at')
    .eq('dte_document_id', document.id)
    .order('attempt_number', { ascending: true })
  if (attemptsError) throw attemptsError

  if ((existingAttempts || []).some((attempt) => !attempt.finished_at)) {
    await supabase.from('dte_documents').update({ status: 'SIGNED', updated_at: new Date().toISOString() }).eq('id', document.id).eq('status', 'TRANSMITTING')
    const error = new Error('Existe un intento de transmisión todavía abierto para este DTE. No se enviará otra vez.')
    error.statusCode = 409
    throw error
  }
  if ((existingAttempts || []).length > 0) {
    await supabase.from('dte_documents').update({ status: 'SIGNED', updated_at: new Date().toISOString() }).eq('id', document.id).eq('status', 'TRANSMITTING')
    const error = new Error('Un DTE de PRODUCCIÓN solo puede tener un intento deliberado desde este endpoint. Si hubo una falla incierta, debe revisarse antes de cualquier acción adicional.')
    error.statusCode = 409
    throw error
  }
  ...
  const { data: attempt, error: attemptError } = await supabase
    .from('dte_transmission_attempts')
    .insert({ dte_document_id: document.id, attempt_number: attemptNumber, request_payload: payload })
    .select('id').single()
  if (attemptError) {
    await supabase.from('dte_documents').update({ status: 'SIGNED', updated_at: new Date().toISOString() }).eq('id', document.id).eq('status', 'TRANSMITTING')
    throw attemptError
  ...
  try {
    ...
    const now = new Date().toISOString()
    const rejectedByMh = error.mhPhase === 'recepcion' && Boolean(error.mhBody)
    ...
    await supabase.from('dte_documents').update({
      status: rejectedByMh ? 'REJECTED' : 'TRANSMISSION_UNKNOWN',
