...
export async function getSaasMasterDashboard({request,supabase}){
 ...
 const rows=(companies||[]).map(company=>{const subscription=subMap.get(company.id)||null;return{...company,users:memberCount.get(company.id)||0,subscription:subscription?{...subscription,plan:planMap.get(subscription.plan_id)||null,vertical:verticalMap.get(subscription.vertical_id)||null}:null}})
 const now=Date.now()
 const metrics={companies:rows.length,active:rows.filter(x=>x.subscription?.status==='active').length,trial:rows.filter(x=>x.subscription?.status==='trial').length,demos:rows.filter(x=>x.demo_mode).length,past_due:rows.filter(x=>x.subscription?.status==='past_due').length,suspended:rows.filter(x=>x.subscription?.status==='suspended').length,activation_pending:rows.filter(x=>x.subscription&&!x.subscription.activation_paid_at).length,activation_collected:rows.filter(x=>x.subscription?.activation_paid_at).reduce((sum,x)=>sum+Number(x.subscription?.activation_paid_amount||0),0),expiring_soon:rows.filter(x=>{const end=x.subscription?.current_period_end||x.subscription?.trial_ends_at;if(!end)return false;const delta=new Date(end).getTime()-now;return delta>=0&&delta<=7*86400000}).length,mrr:rows.filter(x=>x.subscription?.status==='active').reduce((sum,x)=>sum+Number(x.subscription?.plan?.monthly_price||0),0)}
