...
test('preflight informa bloqueos sin exponer secretos', () => {
  ...
  assert.equal(status.transmissionEndpointAvailable, true)
  assert.equal(status.productionConfirmationRequired, true)
  assert.equal(status.duplicateProtection, true)
