-- Evita que borradores o documentos de otro ambiente bloqueen la facturación real de una cotización.
-- Solo los DTE que ya entraron al ciclo fiscal reservan el monto de la cotización.
...
begin
  ...
  where d.company_id = new.company_id
    ...
    and d.id <> new.id
    and coalesce(d.environment,'test') = coalesce(new.environment,'test')
    and upper(coalesce(d.status,'')) in ('SIGNING','SIGNED','TRANSMITTING','PROCESSED','CONTINGENCY');

  if round(v_previous + v_new_total,2) > round(v_quote_total,2) + 0.02 then
    raise exception 'La cotización ya tiene $% comprometidos en DTE del mismo ambiente. El nuevo DTE por $% supera el total aprobado de $%. Saldo máximo disponible: $%.',
