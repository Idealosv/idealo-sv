...
begin
  ...
  if v_account_count<>1 or v_account_id is null then
    update public.dte_documents
      set financial_state='PENDING_CASH_ACCOUNT',
          financial_note=format('DTE aceptado por MH. El saldo $%s no se contabilizó automáticamente porque se requieren exactamente una cuenta %s activa; revisá Caja/Bancos.',v_remaining,v_account_type),
  ...
  if v_account_type='CASH' then
    ...
    where company_id=new.company_id and cash_account_id=v_account_id and status='OPEN'
    order by opened_at desc limit 1;
    if v_session_id is null then
      update public.dte_documents
        set financial_state='PENDING_CASH_SHIFT',
            financial_note=format('DTE aceptado por MH. El saldo $%s no se contabilizó en efectivo porque la caja no tiene turno abierto.',v_remaining),
            financial_posted_at=now()
      where id=new.id;
      return new;
    end if;
...
revoke execute on function public.post_processed_dte_financials() from public, anon, authenticated;
