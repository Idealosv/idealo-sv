...
begin
  ...
  if v_order.status not in ('READY','DELIVERED') then raise exception 'La OT debe estar LISTA antes de confirmar la entrega'; end if;

  select role into v_role from public.company_members where company_id = v_order.company_id and user_id = auth.uid();
  if v_role is null or v_role = 'viewer' then raise exception 'Sin permiso para confirmar la entrega'; end if;
  ...
  if nullif(btrim(coalesce(p_recipient_name,'')), '') is null then raise exception 'Indicá el nombre de quien recibe'; end if;
  if p_delivery_method not in ('PICKUP','DELIVERY','INSTALLATION') then raise exception 'Modalidad de entrega no permitida'; end if;
  ...
  insert into public.deliveries(
    ...
    now(), btrim(p_recipient_name), p_notes, now()
  )
