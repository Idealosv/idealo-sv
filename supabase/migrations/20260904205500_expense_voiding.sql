alter table public.expenses
  ...
  add column if not exists void_reversal_id uuid references public.cash_movements(id);
...
drop trigger if exists trg_reverse_voided_business_expense on public.expenses;
create trigger trg_reverse_voided_business_expense
before update of status on public.expenses
for each row execute function public.reverse_voided_business_expense();
