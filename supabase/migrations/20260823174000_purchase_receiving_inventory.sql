...
-- Recepciones parciales/totales, idempotencia, Kardex PURCHASE_IN y costo promedio.

-- Queda un solo trigger canónico de movimiento. El AFTER heredado duplicaba efectos.
...
begin
  ...
  if p.procurement_status not in ('ORDERED','PARTIAL_RECEIVED') then
    raise exception 'La compra debe estar ordenada antes de recibir materiales';
  end if;
  if li.inventory_item_id is null then raise exception 'La partida no está vinculada a un artículo de Inventario'; end if;

  -- Idempotencia: una reintento con la misma clave no vuelve a mover stock.
  ...
  insert into public.inventory_movements(
    ...
    'COM-'||coalesce(p.number::text,p.id::text),
    coalesce(p_notes,'Recepción de compra · '||li.description),auth.uid()
  ...
  from public.purchase_items where purchase_id=p.id;
