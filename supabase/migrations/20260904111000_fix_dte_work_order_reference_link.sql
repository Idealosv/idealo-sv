-- Corrige el patrón de OT para no interpretar el segmento COT-# como orden de trabajo.
...
begin
  ...
  if btrim(v_reference) = '' then return new; end if;
  ...
  if v_quote_token is not null then
    ...
    begin v_quote_number := split_part(v_quote_token, '-', 2)::bigint;
    exception when others then v_quote_number := null; end;
  end if;
  ...
  begin
    v_work_order_number := substring(v_reference from '(?i)(?:^|[^A-Za-z0-9])OT-([0-9]+)')::bigint;
...
-- Reintenta el enlace de documentos no procesados existentes.
