create or replace function public.next_dte_control_number(p_company_id uuid, p_dte_type text, p_environment text default 'test'::text)
returns text
language plpgsql
security definer
...
begin
  ...
  select coalesce(nullif(c.establishment_code,''),'M001'), coalesce(nullif(c.point_of_sale_code,''),'P001')
  into v_establishment,v_pos
