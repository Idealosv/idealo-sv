-- Compatibilidad con el esquema histórico de anticipos ya existente en IDEALO SV.
-- Estados válidos: PENDING, PARTIALLY_APPLIED, APPLIED, VOID.
...
begin
  ...
  for v_advance in
    ...
    where a.company_id=new.company_id and a.client_id=new.client_id and a.status in ('PENDING','PARTIALLY_APPLIED')
