...
using (exists (
  ...
  where cm.company_id = product_variants.company_id and cm.user_id = (select auth.uid())
))
with check (exists (
  select 1 from public.company_members cm
  where cm.company_id = product_variants.company_id and cm.user_id = (select auth.uid())
...
using (exists (
  select 1 from public.company_members cm
  where cm.company_id = product_price_tiers.company_id and cm.user_id = (select auth.uid())
))
with check (exists (
  select 1 from public.company_members cm
  where cm.company_id = product_price_tiers.company_id and cm.user_id = (select auth.uid())
